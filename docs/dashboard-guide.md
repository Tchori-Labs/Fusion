...
- mailbox now includes an **Approvals** tab with pending and history filters (`approved` / `denied` / `completed`), approval detail context, and inline approve/deny actions for pending requests; its unscoped API queue is global across visible projects and each row provides `projectId` for the explicitly scoped follow-up decision
