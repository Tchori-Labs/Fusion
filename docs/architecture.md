...
- **Creation title invariant (KB-009)**: both shared task insert helpers resolve titles with `resolveCreatedTaskTitle`: an explicit usable title, then a drift-normalized description first line, then `FALLBACK_TASK_TITLE`. Therefore no normal creation path persists a blank title. Deferred AI summarization may replace only that derived placeholder; refine-task derivation and terminal-triage-failure backfill remain defense-in-depth for their existing paths.
