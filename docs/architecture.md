...
- Retry-exhausted durable-agent rearm (KB-012): `SelfHealingManager.rearmExpiredRetryExhaustedAgents()` runs in Batch 2 maintenance and is the only steady-state re-entry for recoverable `paused/error-retry-exhausted` agents; timers continue to exclude paused states. It reads elapsed `durableErrorRecovery.nextRetryAt`, or a parseable `updatedAt` after `DURABLE_RETRY_EXHAUSTED_REARM_FLOOR_MS` (15 minutes) when that metadata was cleared/absent, before resetting shared/legacy retry metadata, clearing error and pause state, re-arming the heartbeat, and emitting `agent:rearm-error-retry-exhausted`. The predicate preserves ephemeral, disabled-runtime, active-execution, manual/user-pause, error-unrecoverable, heartbeat-model-unavailable, operator-actionable, and stale worktree/module-resolution suppressions.
...
- **Retry-exhausted durable-agent rearm (KB-012)**: periodic Batch 2 maintenance owns the steady-state exit from a recoverable `paused/error-retry-exhausted` park. `rearmExpiredRetryExhaustedAgents()` reads an elapsed `durableErrorRecovery.nextRetryAt`, or waits for `DURABLE_RETRY_EXHAUSTED_REARM_FLOOR_MS` (15 minutes) after parseable `updatedAt` when the timestamp was cleared/absent, then resets shared/legacy retry metadata, clears `lastError`/pause state, flips the agent to `active` before re-arming its heartbeat, and emits `agent:rearm-error-retry-exhausted`. The normal heartbeat timer remains paused-state-excluding; this sweep preserves ephemeral, disabled-runtime, active-execution, manual/user-pause, error-unrecoverable, heartbeat-model-unavailable, operator-actionable, and stale module/worktree suppressions.
