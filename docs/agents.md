...
`SelfHealingManager.rearmExpiredRetryExhaustedAgents()` is the steady-state exit for a recoverable durable agent parked at `pauseReason="error-retry-exhausted"`. Its Batch 2 maintenance pass reads `durableErrorRecovery.nextRetryAt`: once elapsed, it resets the shared/legacy retry metadata, clears `lastError` and the pause reason, moves the agent to `active`, records `agent:rearm-error-retry-exhausted`, and re-arms its heartbeat. If an operator cleared or legacy data lacks that timestamp, the same path waits for the fixed `DURABLE_RETRY_EXHAUSTED_REARM_FLOOR_MS` (15 minutes) from a parseable `updatedAt`; absent or malformed timestamps remain suppressed. It preserves the ephemeral, runtime-disabled, active-execution, operator-actionable, stale worktree/module-resolution, `error-unrecoverable`, `heartbeat-model-unavailable`, and manual/user-pause suppressions. The heartbeat timer deliberately still does not arm paused agents; this sweep is their controlled re-entry point.
...
**Operator recovery lever:** `POST /api/agents/:id/state` with `{"state":"active"}` resumes an agent immediately through `HeartbeatMonitor`; the `fn_agent_start` tool exposes the corresponding operator action. This resumes the agent but does not itself reset the shared retry budget, so the periodic re-arm path remains the safe automatic recovery for expired retry-exhausted parks.
