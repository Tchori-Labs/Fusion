...
- Startup/store-open allocator reconciliation bumps each active prefix sequence to `max(current nextSequence, max(tasks suffix)+1, max(project archivedTasks suffix)+1, max(cold-storage archivedTasks suffix)+1, max(reservation sequence)+1)` so stale allocator rows self-heal before local task creation resumes. IDs held only by `archive.archived_tasks` remain reserved after archive cleanup hard-deletes their live task row.
