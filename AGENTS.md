...
- KB-012: periodic self-healing emits `agent:rearm-error-retry-exhausted` when an eligible `paused/error-retry-exhausted` agent reaches its recorded retry cooldown (or the fixed missing-metadata floor), resets shared/legacy retry state, clears error/pause state, and re-arms the heartbeat. Metadata is ids/counts/outcomes-only (`agentId`, `attempts`, `limit`, `priorState`, `priorPauseReason`, `reason`, `source:"self-healing"`) and never persists `lastError` prose.
